...
def generate_csv_from_test_summaries(testSummaries, filename="test_results.csv"):
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # Write header row
        writer.writerow([
            "Training Set Size", "Testing Set Size", "Num Segments", "Distance Mode",
            "k", "Good Matches", "Bad Matches", "Accuracy", "Confusion Matrix"
        ])

        for test_summary in testSummaries:
            training_set_size, testing_set_size, num_segments, distance_mode, k_summaries = test_summary
            ...
            for k_summary in k_summaries:
                ...
                writer.writerow([
                    training_set_size, testing_set_size, num_segments, distance_mode,
                    k, good_matches, bad_matches, accuracy, "Confusion Matrix:"
                ])

                # Prepare header row for confusion matrix
                header_row = ["", "", "", "", "", "", "", "", "Predicted/Actual"] + list(range(10))
                writer.writerow(header_row)

                # Write confusion matrix rows with row labels
                for row_index, row in enumerate(summary_matrix):
                    matrix_row = ["", "", "", "", "", "", "", "", str(row_index)] + list(row)
...
def test():
    ...
    for testCase in testCases:
        ...
        for k in range(knnRangeStart, knnRangeEnd+1):
            ...
            kSummary.append([k, good_match, bad_match, good_match/(good_match+bad_match), summaryMatrix])
            print(k, good_match, bad_match, good_match/(good_match+bad_match))

        testSummaries.append([trainingSetSize, 10000-trainingSetSize, numSegments, distanceMode, kSummary])
