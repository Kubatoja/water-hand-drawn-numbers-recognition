import copy
import itertools
from typing import List

from Testers.XgBoostTester.configs import XGBTestConfig, FieldConfig


def create_xgb_test_configs(
    field_configs: List[FieldConfig],
    generate_combinations: bool,
    default_config: XGBTestConfig
) -> List[XGBTestConfig]:
    """
    Tworzy listę obiektów XGBTestConfig na podstawie konfiguracji pól.

    Args:
        field_configs: Lista obiektów FieldConfig zdefiniowanych dla każdego pola
        generate_combinations: Jeśli True, generuje wszystkie kombinacje. 
                              Jeśli False, zmienia tylko jeden parametr na raz
        default_config: Obiekt XGBTestConfig z domyślnymi wartościami

    Returns:
        Lista obiektów XGBTestConfig zawierająca testy
    """

    # Przygotowanie wartości dla każdego pola
    field_values = {}
    for config in field_configs:
        if isinstance(config.start, (int, float)):
            # Obsługa zakresów float
            if isinstance(config.start, float) or isinstance(config.stop, float) or isinstance(config.step, float):
                current = config.start
                values = []
                while current <= config.stop:
                    values.append(round(current, 10))  # Unikanie problemów z precyzją
                    current += config.step
            else:
                # Obsługa zakresów int
                values = list(range(int(config.start), int(config.stop) + 1, int(config.step)))
        else:
            values = [config.start, config.stop]
        field_values[config.field_name.value] = values

    result_configs = []

    if generate_combinations:
        # Generowanie wszystkich kombinacji
        keys = list(field_values.keys())
        combinations = list(itertools.product(*field_values.values()))

        for combo in combinations:
            cfg = copy.deepcopy(default_config)
            for i, key in enumerate(keys):
                setattr(cfg, key, combo[i])
            result_configs.append(cfg)

    else:
        # Generowanie tylko jednego zmienionego parametru na raz
        for key, values in field_values.items():
            for value in values:
                cfg = copy.deepcopy(default_config)
                setattr(cfg, key, value)
                result_configs.append(cfg)

    return result_configs
